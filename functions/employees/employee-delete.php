...
if(checkSession()) {
	...
	if(checkPermission('employee-delete')) {
		...
		if($upass_db == $upass) {
			...
			$sql = $pdo->prepare("SELECT id FROM employees WHERE id = :id LIMIT 1");
			$sql->bindParam(":id",$id);
			$sql->execute();
			$data = $sql->fetch(PDO::FETCH_ASSOC);
			...
			if($sql->rowCount()) {
				...
				$epoch_time = time();
				$sql = $pdo->prepare("UPDATE employees SET temp_del = ".$epoch_time." WHERE id = :id LIMIT 1");
				$sql->bindParam(":id",$id);
				$sql->execute();
				
				// record to system log
				systemLog('employee',$id,'delete','');
...
renderConfirmDelete($err_code,'sys_employees_suc','employees_messages_employee_removed');
?>