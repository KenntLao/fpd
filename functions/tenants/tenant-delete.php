...
if(checkSession()) {
	...
	if(checkPermission('tenant-delete')) {
		...
		if($upass_db == $upass) {
			...
			$sql = $pdo->prepare("SELECT id FROM tenants WHERE id = :id LIMIT 1");
			$sql->bindParam(":id",$id);
			$sql->execute();
			$data = $sql->fetch(PDO::FETCH_ASSOC);
			...
			if($sql->rowCount()) {
				...
				$epoch_time = time();
				$sql = $pdo->prepare("UPDATE tenants SET temp_del = ".$epoch_time." WHERE id = :id LIMIT 1");
				$sql->bindParam(":id",$id);
				$sql->execute();
				
				// record to system log
				systemLog('tenant',$id,'delete','');
...
renderConfirmDelete($err_code,'sys_tenants_suc','tenants_messages_tenant_removed');
?>