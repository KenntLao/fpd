...
if(checkSession()) {
	...
	if(checkPermission('pre-operation-audit-TSA-add')) {
        ...
        if($err == 0) {
            ...
            $sql = $pdo->prepare("INSERT INTO pre_operation_audit_tsa (
                prospect_id, 
                date_of_audit,
                date_presented,
                summary, 
            ...
            // insert in compliances
                // foreach($conformance as $key => $conform) {
                //     $sql = $pdo->prepare("INSERT INTO pre_operation_audit_tsa_compliances (
                //         tsa_id, 
                //         conformance, 
                //         status, 
                //         remarks
                ...
                //         $sql->bindParam(":conformance", $conform);
                //         $sql->bindParam(":conformance_status", $conformance_status[$key]);
                //         $sql->bindParam(":conformance_remarks", $conformance_remarks[$key]);
                //         $sql->execute();
                //     }
                // }
            // 
            ...
            foreach($location_category as $cat_key => $cat) {

                $sql = $pdo->prepare("INSERT INTO pre_operation_audit_tsa_system (
                    tsa_id, 
                    category, 
                    details,
                    notes
                ...
                $sql->bindParam(":details", $location_details[$cat_key]);
                $sql->bindParam(":notes", $location_notes[$cat_key]);
                $sql->execute();

                $system_id = $pdo->lastInsertId();

                // insert system pictures
                    $sql = $pdo->prepare("INSERT INTO pre_operation_audit_tsa_system_pictures (
                        system_id, 
                        picture, 
                        findings,
                        prioritization,
                ...
                // insert system_locations
                    ...
                    foreach($location_code as $key => $code) {
                        ...
                        if($cat == $code) {
                            ...
                            $sql1->bindParam(":remarks", $location_remarks[$key]);
            ...
            // FIRE EXTINGUISHERs AND EMERGENCY LIGHTS INSERT FUNCTION
                foreach ($fire_safety_category as $key => $category) {
                    ...
                    $sql = $pdo->prepare("INSERT INTO pre_operation_audit_tsa_fire_safety_and_security (
                        tsa_id,
                        category,
                        details
                    ) VALUES (
                            ...
                            if(isset($_POST[$code.'_loc_category'])) {
                                ...
                                foreach ($fire_safety_loc_category as $key4 => $loc_category) {
                                    ...
                                    if ($loc_category == $fire_safety_site_category[$key2]) {
                                        ...
                                        //SITE INSERT FUNCTION
                                        $sql3 = $pdo->prepare("INSERT INTO pre_operation_audit_tsa_fire_safety_and_security_site_location (
                                            site_id,
                                            category,
                                            location,
                                            type,
            ...
            // Insert into tsa_permit_licences
                    ...
                    foreach($permit_particulars as $key => $particular) {
                        ...
                        $sql = $pdo->prepare("INSERT INTO pre_operation_audit_tsa_permit_licences (
                            tsa_id, 
                            particulars, 
                            date_of_issuance,
                            findings,
            ...
            // Insert into tsa_as_built_plans
                    ...
                    foreach($as_built_description as $key => $description) {
                        ...
                        if(!empty($description)) {
                            ...
                            $sql = $pdo->prepare("INSERT INTO pre_operation_audit_tsa_as_built_plans (
                                tsa_id, 
                                description, 
                                sheets_available, 
                                recommendation,
                            ...
                            $sql->bindParam(":sheets", $as_built_sheets[$key]);
            ...
            // Insert into tsa_equipment_manuals
                    ...
                    foreach($manual_contractor as $key => $contractor) {

                        if(!empty($contractor)) {

                            $sql = $pdo->prepare("INSERT INTO pre_operation_audit_tsa_equipment_manuals (
                                tsa_id,
                                contractor, 
                                description, 
                                submitted_documents,
                            ...
                            $sql->bindParam(":tsa_id", $tsa_id);
                            $sql->bindParam(":contractor", $contractor);
                            $sql->bindParam(":description", $manual_description[$key]);
                            $sql->bindParam(":submitted_documents", $manual_documents[$key]);
            ...
            systemLog('pre_operation_audit_TSA',$tsa_id ,'add','');

            $employees = getTable("employees");
            $users = getTable("users");
            foreach ($employees as $employee) {
                push_notification('pre-operations-audit-tsa',$tsa_id,$employee['id'],'employee','pre_operations_audit_tsa_add');
            }
            foreach ($users as $user) {
                push_notification('pre-operations-audit-tsa',$tsa_id,$user['id'],'user','pre_operations_audit_tsa_add');
            }

            $_SESSION['sys_pre_operation_audit_tsa_suc'] = renderLang($pre_operation_audit_tsa_added);
            header('location: /pre-operation-audit-tsa-list');

        } else {

            $_SESSION['sys_pre_operation_audit_tsa_add_err'] = renderLang($form_error);
            header('location: /add-tsa-pre-operation-audit');
