...
if(checkSession()) {
	...
	if(checkPermission('client-delete')) {
		...
		if($upass_db == $upass) {
			...
			$sql = $pdo->prepare("SELECT id FROM clients WHERE id = :id LIMIT 1");
			$sql->bindParam(":id",$id);
			$sql->execute();
			$data = $sql->fetch(PDO::FETCH_ASSOC);
			...
			if($sql->rowCount()) {
				...
				$epoch_time = time();
				$sql = $pdo->prepare("UPDATE clients SET temp_del = ".$epoch_time." WHERE id = :id LIMIT 1");
				$sql->bindParam(":id",$id);
				$sql->execute();
				
				// record to system log
				systemLog('client',$id,'delete','');
...
renderConfirmDelete($err_code,'sys_clients_suc','clients_messages_client_removed');
?>