				...
				<div class="container-fluid">
                    ...
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title"><?php echo renderLang($pdc_list); ?></h3>
                        </div>
                        <div class="card-body">
                            ...
                            <div class="table-responsive">
								<table class="table table-bordered table-hover table-condensed">
									...
									<tbody>
                                        <?php 
                                        if($_SESSION['sys_account_mode'] == "user") { // users admin

                                            $sql = $pdo->prepare("SELECT * FROM daily_collections dc JOIN daily_collections_payment_types dcpt ON dc.id = dcpt.daily_collection_id LEFT JOIN sub_properties sp ON sp.id = dc.sub_property_id LEFT JOIN properties p ON sp.property_id = p.id WHERE voucher_type = 3 AND dc.temp_del = 0 GROUP BY sub_property_id");
                                            $sql->execute();
                                            while($data = $sql->fetch(PDO::FETCH_ASSOC)) {
                                                echo '<tr>';
                                                    echo '<td>';
                                                        echo '<a href="/add-pdc-monitoring/'.$data['sub_property_id'].'">'.$data['sub_property_name'].' ['.$data['property_name'].']</a>';
                                                    echo '</td>';
                                                echo '</tr>';
                                            }
											...
											if(empty($cluster_ids)) {
												...
												foreach($sub_properties as $sub_property_id) {
                                                    ...
                                                    $sql = $pdo->prepare("SELECT * FROM daily_collections dc JOIN daily_collections_payment_types dcpt ON dc.id = dcpt.daily_collection_id LEFT JOIN sub_properties sp ON sp.id = dc.sub_property_id LEFT JOIN properties p ON sp.property_id = p.id WHERE voucher_type = 3 AND dc.temp_del = 0 AND dc.sub_property_id = :sub_property_id GROUP BY sub_property_id");
