					...
					<div class="card">
						<div class="card-header">
							...
							<div class="card-tools">
								<!-- <a href="/add-pdc" class="btn btn-danger"><i class="fa fa-plus mr-1"></i><?php echo renderLang($pdc_add); ?></a> -->
							</div>
						</div>
						<div class="card-body">
								...
								<table class="table table-bordered table-hover table-condensed">
									...
									<tbody>
                                        ...
                                        if($_SESSION['sys_account_mode'] == "user") { // users admin
                                            ...
                                            $sql = $pdo->prepare("SELECT * FROM daily_collections dc JOIN daily_collections_payment_types dcpt ON dc.id = dcpt.daily_collection_id LEFT JOIN sub_properties sp ON sp.id = dc.sub_property_id LEFT JOIN properties p ON sp.property_id = p.id WHERE voucher_type = 3 AND dc.temp_del = 0 GROUP BY sub_property_id, collection_date");
                                            $sql->execute();
                                            while($data = $sql->fetch(PDO::FETCH_ASSOC)) {
                                                echo '<tr>';
                                                    echo '<td>';
                                                        echo '<a href="/add-pdc/'.$data['collection_date'].'-'.$data['sub_property_id'].'">'.formatDate($data['collection_date']).'</a>';
                                                    echo '</td>';
                                                    echo '<td>';
