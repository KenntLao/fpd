...
if(checkSession()) {
    ...
    if($sql->rowCount()) {
        ...
        $editable = $_data['status'] == 0 || $_data['status'] == 2 ? 1:0;
    } else {
...
<body class="hold-transition sidebar-mini layout-fixed">
		...
		<div class="content-wrapper">
			...
			<section class="content">
				...
				<div class="container-fluid">
                    ...
                    renderSuccess('sys_count_sheet_edit_suc');
                    ?>

                    <?php if($editable) { ?>
                    <form action="/submit-edit-count-sheet" method="post" enctype="multipart/form-data">
                    <?php } ?>
                                        ...
                                        <div class="table-responsive">
                                            <table class="table">
                                                ...
                                                <tr>
                                                    ...
                                                    <td><input type="text" class="form-control" name="total_overage" value="<?php echo $data['total_overage']; ?>" id="overage"></td>
                                                </tr>
                                            </table>
                                        </div>
                                    </div>
    ...
    $(function(){
       ...
       $('.add-row-undeposited').on('click', function(e){
            ...
            $(this).closest('table').find('tbody').find('tr:nth-last-child(1)').find('input[name="payment_id[]"]').val("0");
        ...
        // undeposited remove row
        $('body').on('click', '.remove-row', function(e){
            e.preventDefault();
            var $this = $(this);
            if($(this).closest('tbody').find('tr').length != 1) {
                if(confirm("Are you sure to permanently delete this row?")) {
                    var id = $this.closest('tr').find('input[name="payment_id[]"]').val();
                    $.post('/delete-undeposited-collection-row', {
                        id:id
                    }, function(data) {
        ...
        $('.add-row').on('click', function(e){
            ...
            $(this).closest('table').find('tbody').find('tr:nth-last-child(1)').find('input[name="check_id[]"]').val("0");
            $(this).closest('table').find('tbody').find('tr:nth-last-child(1)').find('.date').daterangepicker({
                singleDatePicker: true,
                locale: {
                    format: 'YYYY-MM-DD'
