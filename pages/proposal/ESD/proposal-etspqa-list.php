		...
		<div class="content-wrapper">
			...
			<section class="content-header">
				<div class="container-fluid">
					<div class="row mb-2">
						<div class="col-sm-6">
							<h1><i class="fas fa-handshake mr-3"></i><?php echo renderLang($esd_etspqa); ?></h1>
						</div>
					</div>
                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            <?php if(checkPermission('proposal-esd-add')) { ?>
                                <a href="/esd-etspqa-proposal-add" class="btn btn-danger"><i class="fa fa-plus mr-1"></i><?php echo renderLang($etspqa_proposal_esd_add); ?></a>
                            <?php } ?>
                            </div>
                        </div>
                        <div class="card-body">
                            ...
                            <div class="table-resposive">
                                <table id="table-data" class="table table-bordered">
                                    ...
                                    <tbody>
                                        <?php 
                                        $sql = $pdo->prepare("SELECT * FROM proposal_esd_etspqa WHERE temp_del = 0 ORDER BY id DESC");
                                        $sql->execute();
                                        while($data = $sql->fetch(PDO::FETCH_ASSOC)) {
                                            echo '<tr>';
                                                echo '<td>';
                                                    echo '<a href="/esd-etspqa-edit/'.$data['id'].'">'.$data['reference_number'].'</a>';
                                                echo '</td>';
                                                echo '<td>';
                                                ...
                                                if(checkPermission('proposal-esd-edit')) {
                                                    // edit
                                                    echo '<td>';
                                                        echo '<a href="/esd-etspqa-edit/'.$data['id'].'" class="btn btn-success btn-xs" title="'.renderLang($proposals_edit).'"><i class="fa fa-pencil-alt"></i></a>';
                                                        echo '<a href="/esd-etspqa-proposal-print/'.$data['id'].'" class="btn btn-danger btn-xs" title="'.renderLang($proposals_print).'"><i class="fa fa-print"></i></a>';
                                                    echo '</td>';
                                                }
                                            echo '</tr>';
                                        }
