                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            <?php if(checkPermission('proposal-esd-add')) { ?>
                                <a href="/esd-generic-add" class="btn btn-danger"><i class="fa fa-plus mr-1"></i><?php echo renderLang($generic_proposal_esd_add); ?></a>
                            <?php } ?>
                            </div>
                        </div>
                        <div class="card-body">
                            ...
                            <div class="table-resposive">
                                <table id="table-data" class="table table-bordered">
                                    <thead>
                                        ...
                                        <th><?php echo renderLang($proposals_generic_esd_company); ?></th>
                                    ...
                                    <tbody>
                                        <?php 
                                        $sql = $pdo->prepare("SELECT * FROM proposal_esd_generic WHERE temp_del = 0 ORDER BY id DESC");
                                        $sql->execute();
                                        while($data = $sql->fetch(PDO::FETCH_ASSOC)) {
                                            echo '<tr>';
                                                echo '<td>';
                                                    echo '<a href="/esd-generic-proposal/'.$data['id'].'">'.$data['reference_number'].'</a>';
                                                echo '</td>';
                                                echo '<td>';
                                                ...
                                                if(checkPermission('proposal-esd-edit')) {
                                                    // edit
                                                    echo '<td>';
                                                        echo '<a href="/edit-esd-generic-proposal/'.$data['id'].'" class="btn btn-success btn-xs" title="'.renderLang($proposals_edit).'"><i class="fa fa-pencil-alt"></i></a>';
                                                        echo '<a href="/esd-generic-proposal-print/'.$data['id'].'" class="btn btn-danger btn-xs" title="'.renderLang($proposals_edit).'"><i class="fa fa-print"></i></a>';
                                                    echo '</td>';
