...
if(checkSession()) {
	...
	if(checkPermission('proposal-bdd-labor-cost')) {
        ...
        if($sql->rowCount()) {
            ...
            $sql1->bindParam(":parent_id", $_data['id']);
...
<body class="hold-transition sidebar-mini layout-fixed">
		...
		<div class="content-wrapper">
			...
			<section class="content">
				<div class="container-fluid">
                    ...
                    <?php 
                    renderError('sys_labor_cost_outsource_approve_err');
                    renderSuccess('sys_labor_cost_outsource_approve_suc');
                    ?>

					<form action="/submit-edit-outsource-labor-cost" method="post">
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <!-- other version -->
                                <?php 
                                $sql1 = $pdo->prepare("SELECT * FROM labor_cost_outsource WHERE parent_version = :id AND temp_del = 0");
                                $sql1->bindParam(":id", $id);
                                $sql1->execute();
                                $row_count = $sql1->rowCount();
                                $num = 0;
                                ...
                                <h4>
                                    ...
                                    if($status == 3)  {
                                        if($num == $row_count) {
                                            $_version = renderLang($labor_cost_final).' '.renderLang($labor_cost_version);
                                        } else {
                                            $_version = renderLang($labor_cost_version).' '.$data1['version'];
                                        }
                                    } else {
                                        $_version = renderLang($labor_cost_version).' '.$data1['version'];
                                    }
                                    ...
                                    if($num == $row_count && $status != 3) {
                                        echo '<span class="ml-2"><a href="/edit-outsource-labor-cost/'.$data1['id'].'" class="btn btn-xs btn-success" title="'.renderLang($labor_cost_edit).'"><i class="fa fa-pencil-alt"></i></button></a>';
                                    }
                                    ?>
                                </h4>
                                <div class="row">
                                    ...
                                    <div class="col-lg-3 col-md-4 mb-4">
                                        <label for="personnel_night_shift"><?php echo renderLang($labor_cost_needed_personnel_per_night_shift); ?></label>
                                        <input type="number" class="form-control input-readonly personnel_night_shift" name="personnel_night_shift" value="<?php echo $data1['night_shift_personnel']; ?>" readonly>
                                    </div>
                                ...
                                <!-- comment box -->
                                    <?php 
                                    $sql = $pdo->prepare("SELECT * FROM comments WHERE module = 'labor-cost-outsource' AND module_type = 'approval' AND module_id = :id AND temp_del = 0 ORDER BY comment_date DESC");
                                    $sql->bindParam(":id", $id);
                                    $sql->execute();
                                    ?>
                                    <div class="row mt-5">
                            ...
                            <div class="card-footer text-right">
                                ...
                                <?php if(checkPermission('proposal-bdd-labor-cost-edit') && $status == 2) { ?>
                                    <a href="/new-labor-cost-outsource-version/<?php echo $id; ?>" class="btn btn-primary"><i class="fa fa-clone mr-1"></i><?php echo renderLang($labor_cost_new_version); ?></a>
                                <?php } ?>
                            </div>
                        </div>
    ...
    $(function() {
        ...
        if(isset($_SESSION['sys_labor_cost_approve_outsource_suc_alert'])) {
            ?>
            Toast.fire({
                type: 'success',
                title: '<?php echo $_SESSION['sys_labor_cost_approve_outsource_suc_alert']; ?>'
            });
            <?php
            unset($_SESSION['sys_labor_cost_approve_outsource_suc_alert']);
        }
        ?>
        ...
        // approval
            $('body').on('click', '.approval', function(e){
                ...
                $.post("/update-outsource-labor-cost-status", {
                    status:status, id:id
                }, function(data) {
                    switch(data) {
                        case "success":
        ...
        // add comment
            $('#add-comment').on('click', function(e){
                ...
                var module = 'labor-cost-outsource';
