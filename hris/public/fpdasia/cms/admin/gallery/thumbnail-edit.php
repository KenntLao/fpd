<?php 
  ...
  $thumbnail_id=$_GET['gallery_id'];
  $result = mysqli_query($con,"SELECT * FROM table_gallery WHERE gallery_id=$thumbnail_id") or die(mysqli_error());
  $fetch=mysqli_fetch_assoc($result);
    if(isset($_POST['update_thumbnail'])) {
        ...
        if(!isset($_FILES['gallery_image']['tmp_name'])) {
            ...
            $gallery_image_new="gallery/" . $_FILES["gallery_image"]["name"];
            } 
         

    $sql=mysqli_query($con," UPDATE table_gallery SET gallery_image='$gallery_image_new' WHERE gallery_id='$thumbnail_id'");
...
<div class="wrapper">
 ...
 <div class="form-group">
  <img style="margin-left: 20px;margin-right: 20px;" src="<?php echo $fetch['gallery_image'];?>" accept="image/*" width="20%" required>
  </div>

  <div class="form-group col-md-10">
    <label class="text-left font-weight-bold">Update Thumbnail</label>
    <input type="file" name="gallery_image" class="form-control bg-light" required>
  ...
  <button type="submit" class="btn btn-danger px-5" name="update_thumbnail" value="Update"><a href="gallery.php"></a>Update</button>
</div>
</form>
