...
<div class="wrapper">
 ...
 <form action="engineering-upload.php" method="post" enctype="multipart/form-data" name="addroom">
...
<h4><i class="icon fa fa-check"></i> Success!</h4>Image added.</div>
 ...
 <div class="form-group col-md-10">
    ...
    <input type="file" name="es_icon" class="form-control bg-light" placeholder="Image">
  </div>
  <div class="form-group col-md-10">
    <label class="text-left font-weight-bold">Engineering Service  Name</label>
    <input type="name" name="es_name" class="form-control bg-light">
  </div>

  <div class="form-group col-md-10">
    <label class="text-left font-weight-bold">Engineering Service Description</label>
    <textarea type="text" name="es_description" class="textarea" placeholder="Place some text here" style="width: 100%; height: 120px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;"></textarea>
  </div>
  ...
  <button type="submit" class="btn btn-danger px-5" name="submit" value="Upload">Upload</button>
      ...
      $sql = "SELECT * FROM table_engineering_services WHERE es_deleted IS NULL and es_id ORDER BY es_id DESC ";
      $result = mysqli_query($con, $sql);
    ...
    ?>
    <tr><td width="auto">
          <img src="engineering-image/<?php echo $es_icon; ?>" width="auto"></td>
          <td><h4><?php echo $es_name; ?></h4>
          <p><?php echo $es_description; ?></p></td>

    <?php
          echo '<td><a class="btn btn-danger" href=news-and-updates-delete-function.php?es_id=' . $fetch['es_id'] . '">Delete</a>';
          echo '<a class="btn btn-primary" href="engineering-edit.php?es_id=' . $fetch['es_id'] . '">Edit</a></td></tr>';
              }
    ?>
    </table>
    </div>
