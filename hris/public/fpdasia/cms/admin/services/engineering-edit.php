<?php
  ...
  if ($_COOKIE['role']!='Admin') {
   ...
   $es_id=$_GET['es_id'];
  $result = mysqli_query($con,"SELECT * FROM table_engineering_services WHERE es_id=$es_id") or die(mysqli_error());
  $fetch=mysqli_fetch_assoc($result);

    if(isset($_POST['update'])) {

        $es_name_new=$_POST['es_name'];
        $es_description_new=$_POST['es_description'];

          if(!isset($_FILES['es_icon']['tmp_name'])) {
            }
          else{

  $filei=$_FILES['es_icon']['tmp_name'];
  $imagei= addslashes(file_get_contents($_FILES['es_icon']['tmp_name']));
  $image_namei= addslashes($_FILES['es_icon']['name']);
      
      move_uploaded_file($_FILES["es_icon"]["tmp_name"],"engineering-image/" . $_FILES["es_icon"]["name"]);
      
      $es_icon_new=$_FILES["es_icon"]["name"];
            } 

        if(!empty($_FILES['es_icon']['name'])) //new image uploade
          {
            //process your image and data
            $sql=mysqli_query($con," UPDATE table_engineering_services SET 
            es_name='$es_name_new', 
            es_details='$es_description_new',
            es_icon='$es_icon_new'
             WHERE es_id='$es_id'");//save to DB with new image name
          }
        else // no image uploaded
        {
          // save data, but no change the image column in MYSQL, so it will stay the same value
          $sql=mysqli_query($con," UPDATE table_engineering_services SET es_name='$es_name_new', 
            es_description='$es_description_new' WHERE es_id='$es_id'");//save to DB but no change image column
        }
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active">Engineering Service</li>
      </ol>
    </section>
 ...
 <div class="form-group">
  <img style="margin-left: 20px;margin-right: 20px;" src="engineering-image/<?php echo $fetch['es_icon'];?>" accept="image/*" width="auto" required>
  </div>

  <div class="form-group col-md-10">
    <label class="text-left font-weight-bold">Engineering Service Icon</label>
    <input type="file" name="es_icon" class="form-control bg-light">
  
  </div>

  <div class="form-group col-md-10">
    <label class="text-left font-weight-bold">Engineering Service Name</label>
    <input type="text" name="es_name" value="<?=$fetch['es_name']?>" class="form-control bg-light">
  </div>


<div class="col-md-10">
    <label class="text-left font-weight-bold">Engineering Service Description</label>
      <textarea type="text" name="es_description" class="textarea" placeholder="Place some text here" style="width: 100%; height: 120px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;"><?=$fetch['es_description']?></textarea>
  </div>

  <div class="form-group col-md-10">
  <button type="submit" class="btn btn-danger px-5" name="update" value="Update">
    <a  style="color: white;">Update Engineering Service</a></button>
</div>
</form>  
