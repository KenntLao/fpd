...
if ($_COOKIE['role']!='Admin') {
    ...
    if(empty($_COOKIE['idname'])) { 
     ...
     $pm_id=$_GET['pm_id'];
  $result = mysqli_query($con,"SELECT * FROM table_property_management WHERE pm_id=$pm_id") or die(mysqli_error());
  $fetch=mysqli_fetch_assoc($result);
    if(isset($_POST['update'])) {

        $pm_name_new=$_POST['pm_name'];
        $pm_details_new=$_POST['pm_details'];
  ...
  $image_namei= addslashes($_FILES['pm_icon']['name']);
      ...
      move_uploaded_file($_FILES["pm_icon"]["tmp_name"],"pm-icon/" . $_FILES["pm_icon"]["name"]);
      
   $fileh=$_FILES['pm_name_gif']['tmp_name'];
  $imageh= addslashes(file_get_contents($_FILES['pm_name_gif']['tmp_name']));
  $image_nameh= addslashes($_FILES['pm_name_gif']['name']);
      
      move_uploaded_file($_FILES["pm_name_gif"]["tmp_name"],"pm-gif/" . $_FILES["pm_name_gif"]["name"]);

      $pm_image_new=$_FILES["pm_image"]["name"];
      $pm_icon_new=$_FILES["pm_icon"]["name"];
      $pm_name_gif_new=$_FILES["pm_name_gif"]["name"];
            } 


    if (!empty($_FILES['pm_image']['name'])) {


            //process your image and data
            $sql=mysqli_query($con," UPDATE table_property_management SET 
            pm_name='$pm_name_new', 
            pm_details='$pm_details_new',
            pm_icon='$pm_icon_new',
            pm_name_gif='$pm_name_gif_new',
            pm_image='$pm_image_new'
             WHERE pm_id='$pm_id'");
          }
        else // no image uploaded
        {
          // save data, but no change the image column in MYSQL, so it will stay the same value
          $sql=mysqli_query($con," UPDATE table_property_management SET pm_name='$pm_name_new', 
            pm_details='$pm_details_new' WHERE pm_id='$pm_id'");//save to DB but no change image column
        }

    

      header('location: property-management.php?edit_success');
...
<div class="wrapper">
  ...
  <aside class="main-sidebar">
    ...
    <section class="sidebar">
      ...
      <ul class="sidebar-menu" data-widget="tree">
        ...
        <li><a href="property-management.php"><i class="ion ion-folder"></i> <span>Properties</span></a></li>
 ...
 <div class="form-group col-md-10">
  <img style="margin-left: 20px;margin-right: 20px;" src="pm-icon/<?php echo $fetch['pm_icon'];?>" accept="image/*" width="50px"><br>
 
    <label class="text-left font-weight-bold">Properties Icon</label>
    <input type="file" name="pm_icon" value="<?=$fetch['pm_icon']?>" class="form-control bg-light">
  </div>

   <div class="form-group col-md-10">
  <img style="margin-left: 20px;margin-right: 20px;" src="pm-image/<?php echo $fetch['pm_image'];?>" accept="image/*" width="50px"> <br><br>
 
    <label class="text-left font-weight-bold">Properties Image</label>
    <input type="file" name="pm_image" value="<?=$fetch['pm_image']?>" class="form-control bg-light">
  </div>

    <div class="form-group col-md-10">
  <img style="margin-left: 20px;margin-right: 20px;" src="pm-gif/<?php echo $fetch['pm_name_gif'];?>" accept="image/*" width="50px"> <br><br>
 
    <label class="text-left font-weight-bold">Properties Name GIF</label>
    <input type="file" name="pm_name_gif" value="<?=$fetch['pm_name_gif']?>" class="form-control bg-light">
  </div>

  <div class="form-group col-md-10">
    <label class="text-left font-weight-bold">Properties Name</label>
    <input type="text" name="pm_name" value="<?=$fetch['pm_name']?>" class="form-control bg-light">
  </div>

  <div class=" col-md-10">
    <label class="text-left font-weight-bold">Properties Details</label>
      <textarea type="text" name="pm_details" class="textarea" placeholder="Place some text here" style="width: 100%; height: 400px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;"><?=$fetch['pm_details']?></textarea>
 </div>

  <div class="form-group col-md-10">
    <a href="property-management.php" class="btn btn-primary mx-3">Back</a>
